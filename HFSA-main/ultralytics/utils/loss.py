...
class SemanticSegmentationLoss:
    ...
    def __init__(self, model, ignore_index=255):
        """Initialize semantic segmentation cross-entropy loss."""
        self.device = next(model.parameters()).device
        self.ignore_index = int(ignore_index)
        self.criterion = nn.CrossEntropyLoss(ignore_index=self.ignore_index)
        self.pos_weight_max = float(getattr(model, "loss_pos_weight_max", 20.0))
        self.small_target_weight = max(float(getattr(model, "loss_small_target_weight", 1.0)), 1.0)
    ...
    def __call__(self, preds, batch):
        ...
        if masks is None:
            raise KeyError("Semantic segmentation batch requires a 'mask' tensor shaped [B, H, W].")
